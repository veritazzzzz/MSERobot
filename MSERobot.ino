...
const int ci_Back_Ultrasonic_Data = 18;
const int ci_Left_Ultrasonic_Data = 13;
const int ci_Right_Ultrasonic_Data = 5;

const int ci_Front_Ultrasonic_Ping = 2;
const int ci_Back_Ultrasonic_Ping = 19;
const int ci_Left_Ultrasonic_Ping = 12;
...
unsigned long rightDuration, leftDuration, frontDuration, backDuration; //used to send a length of pings out

const int delayTime = 5; //milisecond time
...
void pingBack() {
  ...
  //delayMicroseconds(delayTime); //used delay time so user could insert how long the ping is used for?? While it is driving along the left wall??
  int backDuration = pulseIn(ci_Back_Ultrasonic_Data, HIGH);
  cmBack = microsecondsToCentimeters(backDuration);
  //Serial.println(backDuration);
  Serial.print(cmBack);
  Serial.print("cm");
  Serial.println();
...
void pingLeft() {
  ...
  //delayMicroseconds(delayTime); //used delay time so user could insert how long the ping is used for?? While it is driving along the left wall??
  int leftDuration = pulseIn(ci_Left_Ultrasonic_Data, HIGH);
...
void pingRight() {
  ...
  delayMicroseconds(delayTime); //used delay time so user could insert how long the ping is used for?? While it is driving along the left wall??
  int rightDuration = pulseIn(ci_Right_Ultrasonic_Data, HIGH);
